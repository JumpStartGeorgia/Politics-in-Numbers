namespace :delayed_job do
  ...
  task stop: :environment do
    ...
    queue! %(cd #{full_current_path} && RAILS_ENV=#{rails_env} #{delayed_job} #{delayed_job_additional_params} stop --pid-dir='#{full_shared_path}/#{delayed_job_pid_dir}')
  end
  ...
  task restart: :environment do
    ...
    queue! %(cd #{full_current_path} && RAILS_ENV=#{rails_env} #{delayed_job} #{delayed_job_additional_params} restart -n #{delayed_job_processes} --pid-dir='#{full_shared_path}/#{delayed_job_pid_dir}')
  end

  desc 'delayed_job status'
  task status: :environment do
    queue %(echo "-----> Delayed job status")
    queue! %(cd #{full_current_path} && RAILS_ENV=#{rails_env} #{delayed_job} #{delayed_job_additional_params} status --pid-dir='#{full_shared_path}/#{delayed_job_pid_dir}')
  end
